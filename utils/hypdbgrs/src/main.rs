...
nix::ioctl_write_ptr!(count_shared_ioctl, MAGIC, COUNT_SHARED, IoctlParams);
nix::ioctl_write_ptr!(print_s2_mappings_ioctl, MAGIC, PRINT_S2_MAPPINGS, IoctlParams);
nix::ioctl_write_ptr!(ramlog_ioctl, MAGIC, PRINT_RAMLOG, IoctlParams);
...
fn dump_print_ramlog(input: &mut File, output: &String) {
        let mut buffer = vec![0; 64];
        let mut dump_file = File::create(&output).unwrap();
        loop {
            let len = input.read(&mut buffer).expect("Read from kernel overflows buffer!");
            if len == 0 {
                break;
            }
...
fn main() -> Result<(), core::fmt::Error> {
    ...
    match args.call {
        ...
        Calls::Ramlog(args) => {
            ...
            //print_kernel_buffer(&mut file);
            if args.dump {
                dump_print_ramlog(&mut file, &args.ofile_log);
            }
        },
    }
    return Ok(());
